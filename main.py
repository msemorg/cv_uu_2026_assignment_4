...
from data_loader import CatDogDataset, train_imgs, train_anns, val_imgs, val_anns, transform, INPUT_IMG_SZ
...
def find_optimal_threshold(model, loader, device):
    ...
    with torch.no_grad():
        for images, targets in loader:
            ...
            preds = model(images) # [Batch, 7, 7, 7]
            
            for b in range(preds.shape[0]):
                for i in range(7):
                    for j in range(7):
                        conf = preds[b, i, j, 0].item()
                        gt_obj = targets[b, i, j, 0].item() # 1 if object, 0 if not
...
if TRAIN_NEW_MODEL:
    ...
    # You MUST call this here if you want to use the optimal value!
    best_thresh, best_f1 = find_optimal_threshold(model, val_loader, device)
...
def visualize_inference(loader, model, num_images=4, threshold_value=0.5):
    ...
    for b in range(num_images):
        ...
        # 1. Flatten the 7x7 confidence grid and find the absolute best cell
        ...
        # 2. Extract the data for ONLY this best cell
        ...
        rect = patches.Rectangle((px-pw/2, py-ph/2), pw, ph, 
                                 linewidth=3, edgecolor="lime", facecolor="none")
        axes[b].add_patch(rect)
        axes[b].text(px-pw/2, py-ph/2-5, label_text, color="lime", 
